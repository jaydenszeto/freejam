...
export function initUI(): void {
  ...
  // Icon paths scaled to fill the full 16-unit viewBox the way Spotify's own
  // playbar icons do (otherwise the FreeJam glyph renders ~1px shorter than
  // its neighbours and the whole control row's baseline looks off).
  playbarButton = new api.Playbar.Button(
    "FreeJam",
    `<svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zM6.5 4.5l5 3.5-5 3.5z"/></svg>`,
    () => openModal(),
  );

  // Use Spicetify's built-in shuffle icon so the True Shuffle button shares
  // exactly the same rendering pipeline (and therefore baseline) as Spotify's
  // native shuffle button next to it.
